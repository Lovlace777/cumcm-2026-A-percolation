...
第 4 问把球体纳入同一连通图，并在同一 $90\%$ 约束下比较成本。纯 A 点 $(601,0)$ 成本 $8.92$ 元，$250$ 次试验给出 $P_{\mathrm{on}}=90.8\%$；网格最低成本点 $(570,160)$ 按单价计算为 $8.73$ 元，但 $80$ 次试验的置信下界低于 $90\%$。按体积分数线性叠加的近似与仿真均显示，球体主要连接邻近圆柱，长程通路仍由圆柱承担，故最低成本方案取 $601$ 根介质 A。
\end{abstract}

\medskip
\noindent{\heiti 关键词}\quad 连续渗流；可贯穿圆柱；并查集；增量蒙特卡洛；排除体积；填充成本；边界截断；有限尺度效应
...
由排除体积的细长渐近，$\varphi_c$ 约与 $1/\alpha$ 成正比，本题阈值因此落在 $1\%$ 量级，与第 2 问给出的 $0.50\%\sim 1.00\%$ 区间一致。边界截断把一条空间直线段拆成若干落在 $\Omega$ 内的截断段，须事先约定各段是否视为同一导电体，否则同一物理对象会被重复计数或被错误地同时接到两极。有限尺度 $W=2L$ 使取向涨落强烈，导通所需个数 $N_c$ 的分布会显著右偏，概率估计应同时给出分位与区间。

\subsection{各问的目标与方法}

第 1 问给定附件三组端点，把左右电极与每条圆柱段视为节点，按 $\delta$ 规则连边，用并查集判断左右电极是否属于同一连通分量。第 2 问转入随机投放，在一次投放中按次序加入介质，记录首次出现左右通路的临界个数 $N_c$，再用经验分布函数读出题面四个体积分数上的 $P_{\mathrm{on}}$。第 3 问不另做投放，只在百分号下两位的网格上读取同一条曲线，找出使 $P_{\mathrm{on}}\ge 0.90$ 的最小 $\varphi_A$。第 4 问把球体加入同一张连通图，在 $(N_A,N_B)$ 网格上估计导通概率，并按单价比较满足约束的格点成本。四问共用第 5 节的最短距离计算与并查集，后问只增加投放方式或节点类型，如图\ref{fig:framework} 所示。
...
附件每一行是一条独立的介质 A，随机投放时一根完整圆柱经边界截断得到的各段亦按独立介质处理。该约定与附件说明一致，避免把跨越 $X$ 向边界的同一根完整圆柱同时接到左右电极，渗流阈值随之升高，第 2 问的体积分数区间因此能够区分导通与不导通。
\end{assumption}
...
\]
负值表示轴线距小于 $2R_A$，两段已经贯穿，它们分别把左电极所在连通分量扩大到 $\{1,2,3,4\}$、把右电极所在连通分量扩大到 $\{8,9,10,12\}$，两侧之间再无桥接，故不导通。图\ref{fig:graph} 以段中点为节点、接触为边，把这一分离画成平面图。组 1 左右两团之间没有连接两侧的边，组 2 则由内部接触把两侧电极连成一体。
...
把同一批 $N_c$ 写成经验分布函数，即得整条 $P_{\mathrm{on}}(\varphi)$，见图\ref{fig:ecdf}。直方图给出临界个数的涨落，分布函数给出第 2、3 问所需的概率，二者来自同一组样本，四个题设点落在曲线的起始、腰部与饱和段，菱形标记的 $90\%$ 分位与第 3 问读数一致。
...
整条 $P_{\mathrm{on}}(\varphi)$ 见图\ref{fig:pphi}，曲线在 $0.45\%\sim 0.95\%$ 之间完成从约 $10\%$ 到接近 $1$ 的上升，题面四个点恰好落在起始、腰部与饱和三段，与该长径比的渗流窗口重合。
...
\label{eq:mix}
\end{equation}
给出的等概率线在成本平面上的最低点位于 $\varphi_B=0$，即纯 A。球体的局部连接是否把最优点推离该直线，由下一小节的网格仿真给出。
...
网格内成本最低且满足点估计约束的点为 $(570,160)$，较纯 A 节省 $0.19$ 元（约 $2.2\%$），该点 $80$ 次试验的区间半宽达 $6.6$ 个百分点，下界低于 $0.90$，而纯 A 点在 $250$ 次试验中 $\hat P=0.908$。把 $N_A$ 降到 $500\sim 540$ 后，即使补上 $400$ 颗球，$\hat P$ 仍多在 $0.63\sim 0.85$，未达到 $90\%$ 阈值。

图\ref{fig:costcurve} 把同一网格改画成成本与概率的一组曲线，每条曲线对应固定的 $N_B$。增加球体使曲线向左（更便宜）移动的幅度很小，向上（更高概率）的抬升在 $N_A\le 540$ 时仍低于 $0.90$，越过 $90\%$ 约束的点集中在 $N_A\ge 560$ 且成本位于 $8.7\sim 9.0$ 元的窄带，与线性叠加近似给出的纯 A 最优点一致。
