...
本问题在机制上属于高长径比棒状填料的连续渗流，而不是格点上的键渗流。需要同时处理四类几何对象之间的最短距离：有限圆柱--有限圆柱、有限圆柱--平面、球体--圆柱、球体--平面。圆柱长径比
\[
...
\caption{微构体坐标系、左右带电面以及两类介质的相对尺度示意}
\label{fig:coord}
\end{figure}
...
\label{eq:judge}
\end{equation}
\eqref{eq:judge} 把隧穿、接触与贯穿统一为同一闭不等式，重叠时左端为负，自然判为导通。
...
\caption{组 1、组 2 的接触图（节点取轴线中点）。青绿为跨越簇，蓝、橙为单侧簇，灰为孤立点；左右竖线为带电表面}
\label{fig:graph}
\end{figure}
...
\caption{左：$(N_A,N_B)$ 上的导通概率，红圈为 $\hat P\ge 0.90$ 的格点；右：成本--概率散点，星号为网格内最低成本可行点}
\label{fig:cost}
\end{figure}
