...
介质可出现在微构体内任意位置和任意方向，不计重力，位置一经生成即固定。允许因随机投放而产生相互贯穿与重叠。若某介质与带电表面的最短距离，或与另一已带电介质表面的最短距离不超过
\[
\delta=1.8\,\mathrm{nm},
\]
...
体积分数定义为导电介质几何体积之和与微构体体积之比。若 $\varphi V/V_A$ 不是整数，按题目要求四舍五入。程序与批量结果见附录。

\section{问题分析}

\subsection{核心难点}

本问题属于高长径比棒状填料的连续渗流。需要同时处理四类几何对象之间的最短距离，即有限圆柱与有限圆柱、有限圆柱与平面、球体与圆柱、球体与平面。记圆柱长径比
\[
...
附件每一行是一条独立的介质 A。随机投放时，一条母圆柱经边界截断得到的各子段亦按独立介质处理。该约定与附件说明一致，避免把跨越 $X$ 向边界的同一母圆柱同时接到左右电极。其后果是渗流阈值升高，第 2 问的体积分数区间因此具有分辨力。
\end{assumption}
...
全文长度用 $\mathrm{nm}$，体积分数用百分数，成本用元。第 3 问体积分数精确到百分号下两位。概率报告点估计与 $95\%$ 正态区间。当 $\hat p\in\{0,1\}$ 时改用规则置信区间的退化形式，并同时给出命中次数。距离保留三位小数，概率保留一位百分数，成本保留两位小数。
...
\label{eq:st}
\end{equation}
则 $d_{\mathrm{seg}}=\|\boldsymbol{p}_1+s^*\boldsymbol{d}_1-\boldsymbol{p}_2-t^*\boldsymbol{d}_2\|$。长度为零或两段平行时，把端点投影到对段上取最小距离。点到线段的距离、线段到平面 $x=x_0$ 的距离分别为
\begin{align}
...
极化规则的算法含义见图\ref{fig:polar}。一旦某介质与已带电对象满足 \eqref{eq:judge}，其全部表面立即成为新的带电源，电荷沿接触图作广度优先传播。该过程等价于先把全部合法边加入无向图，再查询 $L$ 与 $R$ 是否同属一连通分量，因此不必显式模拟时间依赖的极化波前。
...
距离核的退化情形按下列优先级处理。若某段两端重合，即长度为零，则退化为点到线段。若两段平行（$ae-b^2$ 低于 $10^{-18}$），则在垂直平面上比较投影区间。端点落在对方延长线之外时，取四个端点到对段的距离最小值。上述分支可用正交相错、共面平行以及端点对端点等解析算例核对到双精度机器误差。
...
平移后的两段在几何上仍来自同一母圆柱，但按假设 3 记为两条独立介质。若把它们视为同一电学对象，则凡在 $X$ 向越界的母圆柱将同时接触左右电极，单棒短路概率约为 $1/4$，第 2 问四个体积分数的导通概率将全部接近 $1$，与题面把提问集中在 $0.50\%\sim 1.00\%$ 的安排不符。分段独立是使问题适定的关键约定。
...
三组统计量见表\ref{tab:p1}，空间构型与连通着色见图\ref{fig:g3d}。组 1 与组 2 的介质落在 $|Y|,|Z|\le 500\,\mathrm{nm}$ 的薄层内，图中把这两组的 $Y$、$Z$ 方向局部放大，否则在全尺度立方体中会被压成一条线。组 3 按真实立方体绘制。
...
该值介于模拟中位数 $0.70\%$ 与 $90\%$ 可靠阈值 $0.85\%$ 之间，见图\ref{fig:theory}。中位数对应渗流在半数样本中已经出现，可靠阈值对应题目要求的高概率约束。有限盒子 $W=2L$ 使跨越更困难，可靠阈值略高于无限体系的 $\varphi_c^{\mathrm{th}}$，方向与有限尺度理论一致。
...
完全内含对照系综（图\ref{fig:pphi} 虚线）缺少截断产生的贴面短段，触极更困难，$P_{\mathrm{on}}$ 整体右移，其 $90\%$ 阈值升至约 $1.25\%$。主模型与附件数据形态一致。对照表明，若禁止越界回填，填充量需求会明显上升。两种系综的定性排序不变，组 1 仍最稀疏，阈值仍位于百分位。
...
若实际工艺允许同一母圆柱在截断后仍保持电学连通，例如导电涂层连续跨过边界回填缝，则应把同母圆柱的子段预先合并。此时 $P_{\mathrm{on}}(\varphi)$ 会整体左移，第 3 问的可靠填充量将低于 $0.85\%$。本文以分段独立为主模型。附件把每一行写成独立介质，题面又把提问窗口放在 $0.50\%\sim 1.00\%$，只有分段独立才能让这个窗口具有分辨力。
