...
第 4 问把球体纳入同一连通图。纯 A 可行点成本为 $8.92$ 元，网格搜索得到的混合点 $(570,160)$ 标称成本为 $8.73$ 元，节省约 $2\%$，但 $80$ 次试验的置信下界低于 $90\%$。线性混合规则与仿真均表明球体的接头作用不足以替代数十根棒的长程跨越，综合样本量和区间下界，推荐只填充 $601$ 根介质 A。
\end{abstract}
...
渗流阈值预期落在百分位量级，与第 2 问给出的 $0.50\%\sim 1.00\%$ 区间一致。边界截断把一条空间直线段拆成若干落在 $\Omega$ 内的子段，必须事先约定子段的电学归属，否则同一物理对象会被重复计数或被错误地同时接到两极，同时有限尺度 $W=2L$ 使取向涨落强烈，导通所需个数 $N_c$ 的分布会显著右偏，概率估计不能只看均值。
...
平移后的两段在几何上仍来自同一母圆柱，但按假设 3 记为两条独立介质。若把它们视为同一电学对象，则凡在 $X$ 向越界的母圆柱将同时接触左右电极，单棒短路概率约为 $1/4$，第 2 问四个体积分数的导通概率将全部接近 $1$，与题面把提问集中在 $0.50\%\sim 1.00\%$ 的安排不符，因此分段独立是使问题适定的关键约定。
...
对每一组，算法先读取端点，再计算每段到 $x=\pm W/2$ 的距离并把触极介质挂到电极，随后双重循环计算段段距离并做并查集合并，最后比较 $\mathrm{Find}(L)$ 与 $\mathrm{Find}(R)$。组 3 有 $535$ 段，成对计算约 $1.4\times 10^5$ 次，双精度下可在毫秒量级完成，不需空间散列。
...
对 $N_A=495$，另用非增量方式独立生成 $80$ 个构型，并对每个构型调用第 1 问的全量判定，得到 $43/80=0.538$，与表\ref{tab:p2} 的 $0.520$ 在抽样误差内一致。增量算法只计算新加入介质与已有对象之间的距离，上述对照表明没有漏掉旧介质之间的边，把 $M$ 从 $100$ 增到 $250$，$\hat P(495)$ 由 $0.51$ 变为 $0.52$，点估计已稳定。
...
球体进入同一张连通图，按 \eqref{eq:judge} 的 B--B、B--电极、A--B 规则连边。A--B 的捕获半径 $231.8\,\mathrm{nm}$ 约为 A--A 的 $3.8$ 倍，球体把尚未接触的近邻棒桥接起来，自身并不单独承担长程跨越。
...
网格内成本最低的可行点为 $(570,160)$，较纯 A 节省 $0.19$ 元（约 $2.2\%$），该点的 $80$ 次区间半宽达 $6.6$ 个百分点，下界低于 $0.90$，而纯 A 点在第 2 问的 $250$ 次试验中 $\hat P=0.908$，证据更充分。把 $N_A$ 降到 $500\sim 540$ 后，即使补上 $400$ 颗球，$\hat P$ 仍多在 $0.63\sim 0.85$，不足以跨过 $90\%$ 阈值，说明球体的接头作用存在，但在本题尺度上不能替代数十根棒的长程跨越。

图\ref{fig:costcurve} 把同一网格改画成成本与概率的族曲线，每条曲线对应固定的 $N_B$。增加球体使曲线向左（更便宜）移动的幅度很小，而向上（更高概率）的抬升在 $N_A\le 540$ 时始终到不了 $0.90$，真正跨线的点都集中在 $N_A\ge 560$ 且成本靠近 $8.7\sim 9.0$ 元的窄带里，球体单价更低，但不能在本题尺度上替代棒的长程跨越，这一判断可以从成本轴上的定量比较直接读出。
...
\begin{enumerate}
  \item 稳健方案（推荐）。只填充介质 A，$N_A=601$，$\varphi_A=0.85\%$，$N_B=0$，成本 $8.92$ 元。该点由 $250$ 次增量试验支撑，$P_{\mathrm{on}}$ 点估计 $90.8\%$。
  \item 探索性混合。$(N_A,N_B)=(570,160)$，即 $\varphi_A=0.81\%$，$\varphi_B=0.54\%$，成本 $8.73$ 元。该点标称成本更低，但 $80$ 次试验的置信下界低于 $90\%$。若要求区间下界也超过 $90\%$，应采用稳健方案。
\end{enumerate}
...
为把样本量选择说清楚，以 $N_A=601$ 处的 $\hat P$ 为对象做 $400$ 次 Bootstrap，见图\ref{fig:boot}。当 $M$ 从 $30$ 增到 $250$，区间半宽由约 $0.12$ 收至约 $0.04$，点估计稳定在 $0.90\sim 0.91$ 并始终位于约束线之上，继续增大 $M$ 对决策不再改变，第 3 问的 $0.85\%$ 不会因为多样本而滑到 $0.84\%$ 或 $0.86\%$。
...
250 次合计约 $10^8$ 次线段距离，混合网格约 $10\times 7=70$ 格、每格 $80$ 次，量级与主曲线相当，空间散列或扫描线可以把渐近阶降到 $O(n\log n)$，对本题规模并无必要。
...
隧穿阈值进入排除体积的方式是 $D_{\mathrm{eff}}=2R_A+\delta$，图\ref{fig:sens} 给出 $\varphi_c^{\mathrm{th}}(\delta)$。$\delta$ 在 $1.2\sim 2.4\,\mathrm{nm}$ 内变动时，理论阈值只在 $0.81\%\sim 0.82\%$ 之间微移，因为 $\delta$ 远小于直径，因此第 3 问的 $0.85\%$ 不会因 $\delta$ 的小幅标定误差而跳到相邻的百分号下两位。
...
完全内含对照系综（图\ref{fig:pphi} 虚线）缺少截断产生的贴面短段，触极更困难，$P_{\mathrm{on}}$ 整体右移，其 $90\%$ 阈值升至约 $1.25\%$。主模型与附件数据形态一致，对照表明若禁止越界回填则填充量需求会明显上升，两种系综的定性排序不变，组 1 仍最稀疏，阈值仍位于百分位。
...
模型把题面的极化规则写成与贯穿相容的闭距离不等式，相交与隧穿共用同一判定，第 1 至第 4 问使用同一套距离核与并查集。第 1 问三组结论分别为不导通、导通、导通，随机模型因此不会落在平凡的全通或全断区间。增量 Monte Carlo 用一次投放得到整条概率曲线，第 2、3 问共享同一组 $N_c$ 样本，统计口径一致，排除体积给出的 $0.82\%$ 与模拟分位相互印证。

\subsection{局限}

分段独立使问题适定，同时把同一母圆柱的两段当成电学上断开的对象，阈值偏高，若真实极化能沿导体内部跨越截断缝，阈值会下降。有限盒子 $W=2L$ 带来显著有限尺度效应，外推到宏观块体时 $90\%$ 可靠填充量应更接近无限体系阈值 $\varphi_c^{\mathrm{th}}=0.82\%$。第 4 问未对球体作硬核排斥，高 $\varphi_B$ 时名义体积分数会高估真实占有率，混合点相对纯 A 节省 $0.19$ 元，该差值落在 $80$ 次试验的抽样误差内，本文不以它作为推荐方案。
...
若实际工艺允许同一母圆柱在截断后仍保持电学连通，例如导电涂层连续跨过边界回填缝，则应把同母圆柱的子段预先合并，此时 $P_{\mathrm{on}}(\varphi)$ 会整体左移，第 3 问的可靠填充量将低于 $0.85\%$。本文以分段独立为主模型，因为附件把每一行写成独立介质，题面又把提问窗口放在 $0.50\%\sim 1.00\%$，只有分段独立才能让这个窗口具有分辨力。
