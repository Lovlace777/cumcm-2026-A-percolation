...
附件三组的段长分布（图\ref{fig:lenhist}）直接支持上述解读：组 1、组 2 中满长 $5000\,\mathrm{nm}$ 的段分别只有 $2$ 与 $11$ 条，其余均为被截断的短段；组 3 满长段 $185$ 条，约占 $35\%$。若模型在读取附件时再做一次截断，将把已经回填的短段误拆，破坏与题面数据的一一对应。
